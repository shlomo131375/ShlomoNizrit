  ...
  return (
    <div className="max-w-3xl mx-auto px-6 lg:px-8 py-16">
      ...
      <div className="text-center mb-14">
        <div className="w-16 h-16 rounded-2xl bg-[#d4920a]/10 border border-[#d4920a]/20 flex items-center justify-center mx-auto mb-6">
          <Puzzle className="w-8 h-8 text-[#e5a312]" strokeWidth={1.5} />
        </div>
        <h1 className="text-2xl md:text-3xl font-bold text-t-primary tracking-tight mb-4">
      ...
      {/* Download CTA */}
      <div className="bg-s-base border border-b-subtle rounded-2xl p-6 mb-14 text-center">
        <a
          href={downloadUrl}
          className="inline-flex items-center gap-2 bg-[#d4920a] hover:bg-[#e5a312] text-white px-8 py-4 rounded-full text-sm font-medium transition-all duration-300"
        >
            ...
            return (
              <div key={i} className="flex gap-4">
                <div className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-xl bg-s-base border border-b-subtle flex items-center justify-center shrink-0">
                ...
                <div className="pb-8">
                  <div className="flex items-center gap-2 mb-1.5">
                    <Icon className="w-4 h-4 text-t-dim" strokeWidth={1.5} />
                    <h3 className="text-[15px] font-medium text-t-secondary">{step.title}</h3>
                  </div>
                  <p className="text-sm text-t-dim leading-relaxed">{step.desc}</p>
                  {step.highlight && (
                    <code className="inline-block mt-2 px-3 py-1.5 bg-s-base border border-b-subtle rounded-lg text-sm text-[#e5a312] font-mono" dir="ltr">
                      {step.highlight}
                    </code>
                  )}
                  {step.detail && (
                    <pre className="mt-3 px-4 py-3 bg-s-base border border-b-subtle rounded-xl text-xs text-t-muted leading-relaxed whitespace-pre-wrap font-mono" dir="ltr">
                      {step.detail}
                    </pre>
                  )}
                  {step.link && (
